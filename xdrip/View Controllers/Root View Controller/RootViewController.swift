...
final class RootViewController: UIViewController, ObservableObject {
    ...
    @IBAction func stopTimerButtonClicked(_ sender: Any) {
        DispatchQueue.main.async {
            let attributeString: NSMutableAttributedString =  NSMutableAttributedString(string: "---")
            attributeString.addAttribute(.strikethroughStyle, value: 0, range: NSMakeRange(0, attributeString.length))
            
            self.valueLabelOutlet.attributedText = attributeString
            self.valueLabelOutlet.textColor = .darkGray
            self.progressView.reset()
        }
    }
    
    @IBAction func setupNewValueClicked(_ sender: Any) {
        progressView.reset()
        let randomValue = Int.random(in: 70...180)
        currentValue = "\(randomValue)"
        
        DispatchQueue.main.async {
            let attributeString: NSMutableAttributedString = NSMutableAttributedString(string: "\(self.currentValue)")
            attributeString.addAttribute(.strikethroughStyle, value: 0, range: NSMakeRange(0, attributeString.length))
            self.valueLabelOutlet.attributedText = attributeString
            self.progressView.start(interval: 1.0)
            
            if let intValue = Int(self.currentValue) {
                if 70..<100 ~= intValue {
                    self.valueLabelOutlet.textColor = UIColor.green
                } else if 100..<150 ~= intValue {
                    self.valueLabelOutlet.textColor = UIColor.yellow
                } else {
                    self.valueLabelOutlet.textColor = UIColor.red
                }
            }
        }
    }
